...
function Builder() {
  ...
  this.refreshHoveredBlocks = function () {
    ...
    this.hoveredBlock = dataKeeper_2.getBlockAtPosition(
      mouse.wp.x, mouse.wp.y);
  ...
  this.mouseDown = function (event) {
    if (event.button == 0) {  // Left button down
      ...
      this.clickedBlock = dataKeeper_2.getBlockAtPosition(
        this.x, this.y);
...
function BoxSelection() {
  ...
  this.keyDown = function (event) {
    if (event.ctrlKey || event.altKey) {
      ...
      //   mouse.wp.x, mouse.wp.y, mouse.viewport
      // );
      const hoveredBlock = dataKeeper_2.getBlockAtPosition(
        mouse.wp.x, mouse.wp.y);
